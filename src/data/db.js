// ═══════════════════════════════════════════════════
// NESTLÉ SRI LANKA DMS — Persistent Database Engine
// Uses localStorage so ALL inserts/updates survive refresh
// ═══════════════════════════════════════════════════
...
const SEED = {
  ...
  notifications: [
    { id:1, type:"critical", title:"Route Deviation Detected",      msg:"V003 has deviated 4km from assigned route near Matara bypass.", time:"5m ago",  read:false },
    { id:2, type:"warning",  title:"Delivery D006 Severely Delayed", msg:"Lanka Sathosa, Galle — 90 minutes behind schedule.",           time:"12m ago", read:false },
    { id:3, type:"success",  title:"Route R004 Completed",           msg:"Western Coast Run — 9/9 deliveries completed successfully.",    time:"30m ago", read:true  },
    { id:4, type:"info",     title:"New Order Received — ORD006",    msg:"Arpico Matara requires driver & vehicle assignment.",          time:"1h ago",  read:true  },
    { id:5, type:"warning",  title:"Low Fuel — V003",                 msg:"Vehicle V003 fuel at 40%. Schedule refuelling.",               time:"2h ago",  read:true  },
    { id:6, type:"info",     title:"Dispatch Complete — D004",        msg:"Arpico Kandy shipment dispatched. Kumara on route.",           time:"3h ago",  read:true  },
  ],
  messages: [
...
// ══════════════════════════════════════════════════
// DATABASE ENGINE
// All reads/writes go through this object.
// Every write broadcasts a custom event so any
// component subscribed via useDB() re-renders.
// ══════════════════════════════════════════════════

const LS_KEY  = "nestle_dms_db";
const EV_NAME = "nestle_dms_change";

// ── Load or initialise localStorage ───────────────
function loadDB() {
  try {
...
// ── ID generators ──────────────────────────────────
function nextNumericId(arr)  { return arr.length ? Math.max(...arr.map(x=>Number(x.id)||0)) + 1 : 1; }
function nextPrefixId(arr, prefix) {
  const nums = arr.map(x => parseInt((x.id||"").replace(prefix,""))).filter(Boolean);
  const n    = nums.length ? Math.max(...nums) + 1 : 1;
  return `${prefix}${String(n).padStart(3,"0")}`;
}
function nowTime() { return new Date().toLocaleTimeString("en-LK",{hour:"2-digit",minute:"2-digit"}); }
...
export const DB = {
  ...
  getVehicles()    { return loadDB().vehicles; },
  getDeliveries()  { return loadDB().deliveries; },
  getOrders()      { return loadDB().orders; },
  getRoutes()      { return loadDB().routes; },
  getRetailers()   { return loadDB().retailers; },
  getInventory()   { return loadDB().inventory; },
  getNotifications(){ return loadDB().notifications; },
  getMessages()    { return loadDB().messages; },
  getGpsLogs()     { return loadDB().gpsLogs; },

  // ─ Users ────────────────────────────────────────
  addUser(u) {
    const db = loadDB();
    const newUser = { ...u, id: nextNumericId(db.users), active: true };
  ...
  updateUser(id, fields) {
    const db = loadDB();
    db.users = db.users.map(u => u.id===id ? {...u,...fields} : u);
    saveDB(db);
  },
  deleteUser(id) {
    const db = loadDB();
  ...
  addVehicle(v) {
    const db  = loadDB();
    const newV = { ...v, id: nextPrefixId(db.vehicles,"V") };
  ...
  updateVehicle(id, fields) {
    const db = loadDB();
    db.vehicles = db.vehicles.map(v => v.id===id ? {...v,...fields} : v);
    saveDB(db);
  },
  deleteVehicle(id) {
    const db = loadDB();
  ...
  addDelivery(d) {
    const db   = loadDB();
    const newD = { ...d, id: nextPrefixId(db.deliveries,"D"), created: nowTime(), updated: nowTime(), delay:0, proof:false };
  ...
  updateDelivery(id, fields) {
    const db = loadDB();
    db.deliveries = db.deliveries.map(d => d.id===id ? {...d,...fields, updated:nowTime()} : d);
    saveDB(db);
  },
  deleteDelivery(id) {
    const db = loadDB();
  ...
  addOrder(o) {
    const db   = loadDB();
    const newO = { ...o, id: nextPrefixId(db.orders,"ORD"), created: nowTime(), status:"pending", stock:"pending" };
    db.orders.push(newO);
    saveDB(db);
    return newO;
  },
  updateOrder(id, fields) {
    const db = loadDB();
    db.orders = db.orders.map(o => o.id===id ? {...o,...fields} : o);
    saveDB(db);
  },
  deleteOrder(id) {
    const db = loadDB();
  ...
  addRoute(r) {
    const db   = loadDB();
    const newR = { ...r, id: nextPrefixId(db.routes,"R"), status:"planned" };
    db.routes.push(newR);
    saveDB(db);
    return newR;
  },
  updateRoute(id, fields) {
    const db = loadDB();
    db.routes = db.routes.map(r => r.id===id ? {...r,...fields} : r);
    saveDB(db);
  },
  deleteRoute(id) {
    const db = loadDB();
  ...
  addRetailer(r) {
    const db   = loadDB();
    const newR = { ...r, id: nextPrefixId(db.retailers,"RET"), orders:0, lastDel:"Never", status:"active" };
    db.retailers.push(newR);
    saveDB(db);
    return newR;
  },
  updateRetailer(id, fields) {
    const db = loadDB();
    db.retailers = db.retailers.map(r => r.id===id ? {...r,...fields} : r);
    saveDB(db);
  },
  deleteRetailer(id) {
    const db = loadDB();
  ...
  addInventoryItem(item) {
    const db  = loadDB();
    const newI = { ...item, id: nextPrefixId(db.inventory,"SKU"), available: item.stock - (item.reserved||0) };
    db.inventory.push(newI);
    saveDB(db);
    return newI;
  },
  updateInventoryItem(id, fields) {
    ...
    db.inventory = db.inventory.map(i => {
      ...
      updated.available = updated.stock - updated.reserved;
      updated.status    = updated.available <= 0 ? "out_of_stock" : "available";
      return updated;
    });
    saveDB(db);
  },
  ...
  addMessage(m) {
    const db   = loadDB();
    const newM = { ...m, id: nextNumericId(db.messages), time: nowTime(), read: false };
    db.messages.push(newM);
    saveDB(db);
    return newM;
  },
  ...
  addNotification(n) {
    const db  = loadDB();
    const newN = { ...n, id: nextNumericId(db.notifications), time: "Just now", read: false };
  ...
  // ─ Reset to seed data ───────────────────────────
  reset() {
    saveDB(JSON.parse(JSON.stringify(SEED)));
  },
};
...
export function useDB() {
  ...
  useEffect(() => {
    const refresh = () => setData(loadDB());
    window.addEventListener(EV_NAME, refresh);
    return () => window.removeEventListener(EV_NAME, refresh);
  }, []);
  ...
  return {
    ...
    // ── Mutation helpers ──
    addDelivery:        useCallback(d   => DB.addDelivery(d),        []),
    updateDelivery:     useCallback((id,f) => DB.updateDelivery(id,f), []),
    deleteDelivery:     useCallback(id  => DB.deleteDelivery(id),    []),

    addOrder:           useCallback(o   => DB.addOrder(o),           []),
    updateOrder:        useCallback((id,f) => DB.updateOrder(id,f),  []),
    deleteOrder:        useCallback(id  => DB.deleteOrder(id),       []),

    addVehicle:         useCallback(v   => DB.addVehicle(v),         []),
    updateVehicle:      useCallback((id,f) => DB.updateVehicle(id,f),[]),
    deleteVehicle:      useCallback(id  => DB.deleteVehicle(id),     []),

    addRoute:           useCallback(r   => DB.addRoute(r),           []),
    updateRoute:        useCallback((id,f) => DB.updateRoute(id,f),  []),
    deleteRoute:        useCallback(id  => DB.deleteRoute(id),       []),

    addRetailer:        useCallback(r   => DB.addRetailer(r),        []),
    updateRetailer:     useCallback((id,f) => DB.updateRetailer(id,f),[]),
    deleteRetailer:     useCallback(id  => DB.deleteRetailer(id),    []),

    addInventoryItem:   useCallback(i   => DB.addInventoryItem(i),   []),
    updateInventoryItem:useCallback((id,f) => DB.updateInventoryItem(id,f),[]),
    deleteInventoryItem:useCallback(id  => DB.deleteInventoryItem(id),[]),

    addUser:            useCallback(u   => DB.addUser(u),            []),
    updateUser:         useCallback((id,f) => DB.updateUser(id,f),   []),
    deleteUser:         useCallback(id  => DB.deleteUser(id),        []),

    addMessage:         useCallback(m   => DB.addMessage(m),         []),
    markMessagesRead:   useCallback((f,t) => DB.markMessagesRead(f,t),[]),

    addNotification:    useCallback(n   => DB.addNotification(n),    []),
    markAllRead:        useCallback(()  => DB.markAllRead(),          []),

    resetDB:            useCallback(()  => DB.reset(),               []),
  };
}

// ── Legacy named exports (so existing imports keep working) ──
export const USERS         = DB.getUsers();
export const VEHICLES      = DB.getVehicles();
export const DELIVERIES    = DB.getDeliveries();
export const ORDERS        = DB.getOrders();
export const ROUTES        = DB.getRoutes();
export const RETAILERS     = DB.getRetailers();
export const INVENTORY     = DB.getInventory();
export const NOTIFICATIONS = DB.getNotifications();
export const MESSAGES      = DB.getMessages();
export const GPS_LOGS      = DB.getGpsLogs();
